...
test('Empty shopping cart', async () => {
  ...
  expect(shoppingCart).toEqual({ userId: userId, items: [], totalPrice: 0 });
});
...
test('Remove product all quantity', async () => {
  ...
  expect(shoppingCart).toEqual({ userId: userId, items: [{ product: product2, quantity: 1, totalPrice: 35 }], totalPrice: 35 });
});
